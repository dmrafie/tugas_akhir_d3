...
unsigned long mistInterval = 180000; // Mist Maker ON every 3 min
unsigned long mistDuration = 90000;  // Running 1m 30s while mist maker ON
...
bool readSensorStable(int pin) {
  int highCount = 0;
  for (int i = 0; i < 5; i++) {   // baca 5 kali
    if (digitalRead(pin) == HIGH) highCount++;
    delayMicroseconds(1000); // jeda 1ms antar pembacaan
  }
  return (highCount >= 3); // hasil mayoritas
}
...
void loop() {
  ...
  if (currentTime - lastReadTime >= readInterval) {
    ...
    bool upperSensor = readSensorStable(SENSOR1_PIN);   // HIGH = Water Enough
    bool bottomSensor = readSensorStable(SENSOR2_PIN);  // HIGH = Water Available
